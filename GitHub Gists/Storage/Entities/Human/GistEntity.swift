...
open class GistEntity: _GistEntity {
    ...
    static func predicate(with username: String, id: String) -> NSPredicate {
        return NSPredicate(format: "username == %@ AND id == %@", username , id)
    }

    // MARK: Actions

    static func find(with username: String, id: String,
                     in context: NSManagedObjectContext?) -> GistEntity? {
        let predicate = GistEntity.predicate(with: username, id: id)
        return GistEntity.mr_findFirst(with: predicate, in: context) as? GistEntity
    }
    
    static func findOrCreate(with username: String, id: String,
                                 in context: NSManagedObjectContext?) -> GistEntity {
        let foundGist = find(with: username, id: id, in: context)
        if foundGist != nil {
            return foundGist!
        }

        let createdGist = GistEntity.mr_create(in: context) as! GistEntity
        createdGist.id = id
